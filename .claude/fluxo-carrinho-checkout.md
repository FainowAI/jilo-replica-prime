...
- **`cartLinesAdd` da Storefront pode retornar sucesso silencioso (R55, Sprint 5.0):** O `addLineToShopifyCart` no `shopify.ts` interpreta ausência de `userErrors` como sucesso. Isso é verdade pra produtos `ACTIVE`. Pra produtos `DRAFT` (não deveria mais acontecer pós R54, mas serve de canário), a API retorna sucesso sem criar a linha. O `cartStore.addItem` agora faz verificação pós-add apenas para variant fantasma de frete: re-fetch do cart e confirma que a linha existe. Se aparecer log `[cartStore] CRITICAL` em produção, primeiro suspeito é o produto fantasma ter sido trocado de UNLISTED pra outro status — verificar no Shopify Admin.
