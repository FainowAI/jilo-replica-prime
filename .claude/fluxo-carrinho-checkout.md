...
**R48.3 (HMAC + gzip, 2026-09-25).** ⚠️ Desde a v37 (14/09, HMAC fail-closed) o `shopify-webhook-receiver` recusava TODO webhook real com "Invalid HMAC signature" — nenhum pedido entrou em `orders` entre 14/09 e 25/09 (o #1012 de 25/09 ficou com 8 reenvios em ERR). Causa provada por experimento: a Shopify comprime payloads de pedido (~7 KB, `Content-Encoding: gzip`) e assina o JSON descomprimido; o runtime Deno NÃO descomprime, então `req.text()` devolvia os bytes gzip como UTF-8 (U+FFFD, irreversível) e o HMAC nunca batia. Sondas pequenas (<1 KB, não comprimidas) passavam — por isso o segredo parecia certo (e estava). Fix: `_shared/shopify-webhook-body.ts` — `readRawBody(req)` lê `arrayBuffer()` e descomprime via `DecompressionStream` conforme `Content-Encoding` (gzip/deflate); `verifyShopifyHmac(body, header, secret)` compara em tempo constante (`timingSafeEqual`, base64 inválido ⇒ false). Regra: **nunca `req.text()` direto num receiver assinado**. Diagnóstico reproduzível em `_hml/hmac-probe.ts` (mesmo body com e sem gzip).
