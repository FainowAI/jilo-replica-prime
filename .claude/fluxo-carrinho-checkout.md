...
| `src/components/KitQuantityNotice.tsx` | Aviso visual da R56. Retorna `null` em quantidade válida; em quantidade inválida mostra orientação para completar/remover até o múltiplo de `KIT_STEP`. Usado no `/carrinho` e no `CartDrawer` |
...
**Hard-block de quantidade (R56):** o `canCheckout` também exige `isValidKitQuantity(totalNonShippingItems)`. A regra vem de `src/config/kitQuantity.ts`: abaixo de `KIT_STEP` o pedido segue avulso; a partir de `KIT_STEP`, só múltiplos exatos de `KIT_STEP` avançam para checkout. Quando inválido, `/carrinho` mostra `<KitQuantityNotice />` no resumo lateral e o botão fica disabled com "Complete seu kit de 7". O `CartDrawer` também renderiza `<KitQuantityNotice variant="inline" />` como aviso informativo, mas não bloqueia o botão "Finalizar Compra"; ele continua navegando para `/carrinho`, onde o ajuste acontece.
...
- **🐛 Selecionar PIX trava o hard-block do checkout (bug aberto, descoberto Sprint 5.0):** ao escolher PIX, o `PaymentMethodSelector` aplica o cupom `PIX5`/`PIX3` no Shopify Cart via `applyDiscountCode`. Isso reduz o `subtotalAmount` do Shopify (`18.94 × 0.95 + 10.50 ≈ 28.5`), mas o `totalMatchesShopify` em `Carrinho.tsx` compara contra o `expectedTotal` SEM desconto (29.44) → diff ≈ R$ 0,94 → `canCheckout = false` → botão trava em "Sincronizando frete...". Voltar pra Cartão de Crédito libera. É bug da lógica do hard-block (R52 revisado, Sprint 4.9), que assume `subtotalAmount` sem desconto — falso pra cupons como PIX5. Correção pendente: ao comparar, considerar o desconto de cupom (ex.: comparar contra `totalAmount` quando há cupom, ou subtrair o desconto do `expectedTotal`) sem enfraquecer a proteção contra frete-ausente.
