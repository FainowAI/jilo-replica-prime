...
**R80 (checkoutUrl stale, 2026-09-25).** ⚠️ O cupom PIX5 aplicado no `/carrinho` (`cartDiscountCodesUpdate`) **não chegava ao checkout**: o cart no Storefront fechava em 126,29 com `PIX5 applicable`, mas o checkout abria em 132,93 com o campo de cupom vazio. Causa: `cartStore` gravava o `checkoutUrl` **uma vez, na criação do cart** (`createShopifyCart`) e nunca o renovava; a Shopify assina o `checkoutUrl` (`?key=…`) com o estado do cart, e a doc manda re-requisitar quando "stale". A `CART_FULL_QUERY` já trazia a URL fresca a cada `refreshCartDetails` — e ela era descartada. Prova: mesmo cart, URL viva → checkout 126,29 com tag PIX5; URL guardada → 132,93. Fix: `refreshCartDetails` grava `checkoutUrl: formatCheckoutUrl(cart.checkoutUrl)`; como `applyDiscountCode` chama `refreshCartDetails` logo após aplicar, o botão "Ir para o Checkout" abre a URL que já contém o cupom. Regra: **nunca cachear `checkoutUrl` — renovar a cada leitura do cart.** Teste: `src/lib/checkoutUrl.test.ts`.

**R48.3 (HMAC × chave do app, 2026-09-25).** ⚠️ Desde a v37 (14/09, HMAC fail-closed) o `shopify-webhook-receiver` recusava TODO webhook real com "Invalid HMAC signature" — nenhum pedido entrou em `orders` entre 14/09 e 25/09 (o #1012 de 25/09 esgotou 8 reenvios em ERR). **Causa real, provada recalculando o HMAC sobre o body real capturado em produção (v45 de diagnóstico):** o app `jiloapp` no Dev Dashboard tinha **duas chaves secretas ativas** (Antiga 03/03/2026, Nova 27/05/2026); a Shopify assina os webhooks com a **Antiga** e o cofre do Supabase (`SHOPIFY_CLIENT_SECRET`) tinha a **Nova** — que autentica `client_credentials` normalmente (o token Admin renovava todo dia), por isso "o segredo parecia certo". Falso positivo pelo caminho: uma reprodução em laboratório apontou gzip, mas o body real chega SEM compressão (`enc=none`, 7 KB, JSON íntegro) — reprodução controlada não prova causa em produção sem olhar o tráfego real. Correção operacional: revogar a chave Antiga (a Shopify passa a assinar com a Nova em ≤1 h) ou alinhar o cofre. Código que ficou (correto e mais robusto): `_shared/shopify-webhook-body.ts` — `readRawBody(req)` lê `arrayBuffer()` e descomprime se vier `Content-Encoding` (defesa; hoje não vem); `verifyShopifyHmac(body, header, secret)` compara em tempo constante. Sonda: `_hml/hmac-probe.ts <segredo>` (200 = é o segredo do cofre; para saber com qual a Shopify assina, é preciso o body real + header e recalcular).

**R48.2 (causa raiz real + Path B, 2026-06-30).** ⚠️ Os produtos são `requiresShipping: false` → a Shopify NÃO coleta endereço no checkout e IGNORA qualquer endereço no cart (R48.1 fica inerte). O endereço real é resolvido no backend: `shopify-webhook-receiver` lê o `selected_address_id` (note_attribute), busca em `addresses` (service_role) e popula `orders.shipping_address` (Path B, edge v32). Fica no nosso banco/Uber, não no Admin Shopify. Tê-lo no Admin exigiria Path A (requiresShipping=true + shipping rate).
...
- **Checkout cobrando "Padrão R$ 22,00" (set/2026, pedido real da Julia):** causa = as 26 variantes de marmita estavam com `requiresShipping: true` (alguém marcou "produto físico" no Admin; em 30/06 eram `false`). Com isso a Shopify volta a coletar endereço no checkout e aplica a taxa nativa do perfil "Perfil geral" (zona BR, "Padrão" R$ 22) por cima da variant fantasma → pedidos de 1–6 pagavam frete DUAS vezes e os de 7+ pagavam R$ 22. Fix (24/09/2026): `inventoryItemUpdate(requiresShipping:false)` nas 26 (Admin API). Diagnóstico rápido: `productVariants { inventoryItem { requiresShipping } }` — qualquer `true` fora da variant fantasma é o bug. A taxa "Padrão" R$ 22 continua cadastrada (decisão do dono); o perfil "São Paulo + Grande SP" (0 produtos) é resquício e não faz nada.
- **PIX5 tem que continuar como desconto de PEDIDO (classe ORDER, "todos os itens") — testado em 24/09/2026:** ao converter o PIX5 para "desconto em produtos" restrito aos 26 pratos (para o 5% não incidir na linha "Frete Uber Direct"), a Shopify passou a devolver `PIX5 applicable:false` em carrinhos de 7+ (não combina com o desconto automático de Kit, mesmo com `combinesWith.productDiscounts=true` dos dois lados). Revertido no mesmo minuto. Consequência aceita: o PIX dá 5% também sobre a linha de frete (ex.: 6 pratos 119,88 + frete 16,20 → 129,28). O carrinho exibe o `cart.cost.totalAmount` da Shopify como "total com PIX" (`computePixTotals` em `src/config/pixCoupons.ts`, chamado no `Carrinho.tsx` só quando `shopifyHasShippingLine === !free`; `applyDiscountCode` passou a chamar `refreshCartDetails` para o `cartCost` não ficar velho) — a conta local pode divergir em 1 centavo (Shopify aloca por linha: 7 pratos com Kit → PIX 6,64, não 6,65). O sistema do Uber (`update-shipping-variant-price`) só altera o preço da variante de frete e não depende do cupom.
