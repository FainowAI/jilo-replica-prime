...
- **R54.** O produto fantasma `__internal_shipping` (R39) deve permanecer com status `UNLISTED` no Shopify Admin permanentemente. NÃO usar `DRAFT` (a Storefront API rejeita silenciosamente o `cartLinesAdd` de variants de produtos draft, deixando o cart sem o frete — bug que travou 5 sprints) nem `ACTIVE` (aparece em coleções abertas se a query não filtrar). O status `UNLISTED` (Shopify 2025-10+) é o caminho oficial documentado para "custom pricing items that are added to the cart through code adjustments" — produto vendável via Storefront API apenas quando referenciado por ID direto, invisível em buscas/coleções/recomendações. A edge `supabase/functions/set-product-unlisted` faz essa mudança via Admin GraphQL `productUpdate`, é idempotente (se já está UNLISTED, retorna 200 sem operar). Em Storefront API 2025-07 (versão atual do frontend Jiló), o status UNLISTED é traduzido para ACTIVE em queries — `availableForSale` retorna `true` e `cartLinesAdd` aceita a variant. Sprint 5.0 (Maio 2026).

- **R55.** O método `cartStore.addItem` deve fazer **verificação pós-add** apenas quando o item adicionado é a variant fantasma de frete (`isShippingVariant(variantId) === true`). Após `addLineToShopifyCart` retornar `success: true`, fazer `fetchCartFull` e confirmar que existe uma linha cujo `merchandise.id === item.variantId`. Se NÃO existir, logar `console.error` com payload de diagnóstico (cartId, variantId, contagem de lines, IDs das lines existentes), reverter o `items[]` local pra refletir a falha, e o hard-block do `canCheckout` (R52) impede o checkout naturalmente (variant fantasma ausente no Cart faz o `subtotalAmount` ficar dessincronizado do `expectedTotal`). Itens normais (marmitas, produtos ACTIVE) NÃO precisam dessa verificação — a Storefront é confiável pra esses. Sprint 5.0 introduziu essa verificação como defesa em profundidade contra regressões futuras (cache da Storefront, mudanças de status acidentais, novas versões da API com comportamento diferente).
