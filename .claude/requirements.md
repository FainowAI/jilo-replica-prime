...
## Desconto de kit no carrinho (Sprint 5.1, Junho 2026)

- **R56.** O desconto de kit (Automatic Discount do tipo `DiscountProducts` no Shopify) é exibido no carrinho a partir da **agregação de `line.discountAllocations`** — esse tipo de desconto aloca por linha e NUNCA aparece em `cart.discountAllocations` (que vem vazio). O `cartStore.refreshCartDetails()` percorre `cart.lines.edges`, soma `discountedAmount.amount` agrupando por `title` (fallback "Desconto de kit"), e mescla com eventuais allocations de nível cart (cart-level primeiro, agregados de linha depois) em `cartDiscountAllocations`. Decorrências em `Carrinho.tsx`: (a) o TOTAL exibido usa o subtotal líquido de kit (`productsTotalWithDiscount = subtotal - kitDiscountTotal`); (b) o hard-block do checkout (R52) compara `shopifySubtotal` contra `expectedTotal = (subtotal - kitDiscountTotal) + frete`, já que o `cartCost.subtotalAmount` do Shopify vem com o desconto de kit; (c) a base da estimativa de desconto do PIX no `<PaymentMethodSelector />` é o subtotal com desconto de kit. O **cupom manual** (BEMVINDO10, etc.) permanece aplicado apenas no checkout Shopify — não entra no TOTAL exibido.
