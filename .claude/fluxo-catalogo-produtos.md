...
- **O texto de um prato mora em 3 lugares — corrija os 3 juntos:** (1) `descriptionHtml` da Shopify (cards e parágrafo "Componentes"); (2) metafields `custom.*` (é o que "O que tem na sua marmita" e os accordions da página de produto mostram — corrigir só a descrição deixa a página errada); (3) o cardápio hardcoded em `scripts/generate-seo-files.ts` → `public/llms.txt`/`llms-full.txt`. Editar (3) à mão no script e nos `.txt`: `npm run seo` sem `SHOPIFY_ADMIN_TOKEN` regenera o sitemap sem as URLs de produto.
- **Correção 25/09/2026** (doc do cliente "Informação sobre entrega" no Drive, fichas do ciclo 1): 10 pratos corrigidos na Shopify (backup dos valores antigos em `.claude/.work/correcao-textos-pratos/`). Renomeados só no título, handle mantido: "Tilápia Desfiada" → "Filé de Peixe Desfiado" (`tilapia-desfiada`), "Lasanha Bechamel com Calabresa" → "Lasanha Bechamel com Presunto e Mussarela" (`lasanha-bechamel-com-calabresa`). Lasanha de Brócolis: massa de pastel + leite de amêndoa (alergênico amêndoa). Padrão de tempo: selos/títulos "5 min"; instrução de preparo "5 a 6 minutos" (= metafield `modo_preparo`). Validade padrão: 180 dias.
- **Pendentes com o cliente (25/09):** receita real da Calabresa com Mandioca (o doc veio cortado); alérgicos do Nhoque ao Sugo (vegano, mas diz "Contém leite (creme de milho)") e da Panqueca de Frango e Calabresa (mesmo texto colado); "Low carb" nas duas tilápias/peixe com arroz e feijão; confirmar se a massa de pastel da Lasanha de Brócolis tem ovo e se a soja saiu da receita.
