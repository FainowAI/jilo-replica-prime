...
export default defineConfig(({ mode }) => ({
  ...
  // CSP entra no build só junto com a VR (VITE_VR_ENABLED=true): protege o formulário do cartão (A3)
  // e não muda o site em produção enquanto a VR está desligada.
  plugins: [
    react(),
    mode === "development" && componentTagger(),
    loadEnv(mode, process.cwd(), "VITE_").VITE_VR_ENABLED === "true" && cspPlugin(),
  ].filter(Boolean),
  resolve: {
    alias: {
